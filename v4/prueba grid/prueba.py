...
def imagenes(img,tiles):
    ...
    y_start=0
    y_end=image.height
    step_size_w=int(image.width/tiles[1])
    step_size_h=int(image.height/tiles[0])
    for x in range(0,image.width,step_size_w):
    ...
    for y in range(0,image.width,step_size_h):

        line=((x_start,y),(x_end,y))
        draw.line(line,fill=128)
    del draw
    #image.show()
    image.save('./saved/'+nombre)
    return ('./saved/'+nombre)
...
def obtainData(annotationsFile,imgPath):
    ...
    for x in range(10):
        ...
        imagenes(img,nombre,image_tiles.shape)
...
def detect(model, data,labels_array,dims,images):
    ...
    for i in range(len(p)):
        prueba[i]=imagenes(prueba[i],p[i].shape)

        print(p[i])
        for x in range(p[i].shape[0]):
          for y in range(p[i].shape[1]):
            if (p[i][x][y]==1):
              #print('a')
              paint_areas(prueba[i],x,y,20)
    return y_pred
...
def paint_areas(img,y,x,sizeTiles):
  image=Image.open(img)
  #nombre=img.split('./')[1]
  

  draw=ImageDraw.Draw(image)
  draw.rectangle([(x*sizeTiles+sizeTiles),(y*sizeTiles+sizeTiles),(x*sizeTiles),(y*sizeTiles)],fill=128)#x2,y2,x1,y1
  image.save(img)
...
def main():
    ...
    y_pred=detect(loaded_model,test_tiles,test_labels,test_dims,test_img_path)
