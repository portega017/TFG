...
# READING DATA
# =============================================================================
def obtainTiles(annotationsFile,imgPath):
    ...
    for x in range(30):
        ...
        allTiles.append((tile_array))
        allLabels.append((label_array,image_tiles.shape))
...
def prepare_data(annotations_file,img_path,pkl_data_file,pkl_labels_file):
    [Tiles, Labels]=obtainTiles(annotations_file,img_path)
    start = time.time()
...
def obtainLabels(labels_array):
    labels=[]
    for x in range(len(labels_array)):
        for y in labels_array[x][0]:
            labels.append(y)
    return labels

def obtainData(tiles_array):
    tiles=[]
    for x in range(len(tiles_array)):
        tiles.append(tiles_array[x])
    return tiles
...
def detect(model, data,labels_array):
    ...
    for i in range(10):
        ...
        a=l[b:(labels_array[i][1][0]*labels_array[i][1][1])]
        p.append(a)
        b=b+(labels_array[i][1][0]*labels_array[i][1][1])
        print(b)
    print(p)
    return y_pred
...
def main():
    ...
    prepare_data(test_annotations_file,test_img_path,test_pkl_data,test_pkl_labels)'''
    #==============
    # FITTING MODEL
    #==============
    ...
    y_pred=detect(loaded_model,test_tiles,test_labels)
